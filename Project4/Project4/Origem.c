...
int main() {
	...
	enigma = al_load_font("fonts/Kindergarten.ttf", 30, 0);
	digitado = al_load_font("fonts/Kindergarten.ttf", 80, 0);
	...
	campo = al_load_bitmap("imagens/campo2.png");
	btnAbrirCampo = al_load_bitmap("imagens/botao.png");
	fundo2 = al_load_bitmap("imagens/background2.png");

	voltar = al_load_bitmap("imagens/voltar.png");
	apagar = al_load_bitmap("imagens/apagar.png");


	musicaoff = al_load_sample("sounds/desligada.ogg");
	musicaon = al_load_sample("sounds/ligada.ogg");
	musica = al_load_sample("sounds/musicafundo.ogg");
	musica = al_load_audio_stream("sounds/musicafundo.ogg", 4, 1024);
	al_attach_audio_stream_to_mixer(musica, al_get_default_mixer());
	al_set_audio_stream_playmode(musica, ALLEGRO_PLAYMODE_LOOP);

	musicaFundo = al_load_sample("sounds/musicafundo.ogg");
	botaosample = al_load_sample("sounds/click.ogg");
	sampleVoltar = al_load_sample("sounds/arrow_hit.ogg");
	acerto = al_load_sample("sounds/acertou.ogg");
	erro = al_load_sample("sounds/errou.ogg");

	audio = al_load_bitmap("imagens/comaudio.png");
	...
	al_set_window_title(display, "Ink Escape - Pagina Inicial");
	if (btnAbrirCampo) {
		...
		if (evento.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN)
		{
			if (evento.mouse.x >= (800 / 2) - (400 / 2) && evento.mouse.x <= (800 / 2) - (400 / 2) + 400 && evento.mouse.y >= (600 / 2) - (70 / 2) && evento.mouse.y <= (600 / 2) - (70 / 2) + 70)
			{
				...
				if (!btnJogar) {
					...
					al_set_window_title(display, "Ink Escape - Pergunta 1");
				...
				if (btnAudio)
				{
					...
					audio = al_load_bitmap("imagens/semaudio.png");
			...
			if (btnJogar) {
				...
				else if (evento.keyboard.keycode == ALLEGRO_KEY_ENTER)
				{
					if (strncmp(enigmaCerto, digitadoText, 6) == 0) {
						...
						fundo2 = al_load_bitmap("imagens/acerto.png");
						al_play_sample(acerto, 2.0, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE, NULL);
					}
					else {
						fundo2 = al_load_bitmap("imagens/errou.png");
